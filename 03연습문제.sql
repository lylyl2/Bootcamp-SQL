...
SELECT FIRST_NAME || ' ' || LAST_NAME 이름, DEPARTMENT_ID 부서번호, JOB_ID 직무아이디
FROM EMPLOYEES WHERE DEPARTMENT_ID IN 50;  --WHERE DEPARTMENT_ID = 50

--5. 모든 사원의 이름, 급여 그리고 300달러 인상된 급여를 출력하세요.
SELECT FIRST_NAME || ' ' || LAST_NAME 이름, SALARY 급여, SALARY + 300 인상급여 FROM EMPLOYEES;

--6. 급여가 10000보다 큰 사원의 이름과 급여를 출력하세요.
SELECT FIRST_NAME || ' ' || LAST_NAME 이름, SALARY 급여 FROM EMPLOYEES WHERE SALARY > 10000;

--7. 보너스를 받는 사원의 이름과 직무, 보너스율을 출력하세요.
SELECT FIRST_NAME || ' ' || LAST_NAME 이름, JOB_ID 직무, COMMISSION_PCT 보너스율
FROM EMPLOYEES WHERE  COMMISSION_PCT IS NOT NULL;

--8. 2003년도 입사한 사원의 이름과 입사일 그리고 급여를 출력하세요.(BETWEEN 연산자 사용)
SELECT FIRST_NAME || ' ' || LAST_NAME 이름, HIRE_DATE 입사일, SALARY 급여
...
SELECT FIRST_NAME || ' ' || LAST_NAME 이름, SALARY 급여
FROM EMPLOYEES WHERE DEPARTMENT_ID = 60 ORDER BY SALARY DESC;

--12. 직무아이디가 IT_PROG 이거나, SA_MAN인 사원의 이름과 직무아이디를 출력하세요.
SELECT FIRST_NAME || ' ' || LAST_NAME 이름, DEPARTMENT_ID 직무아이디
FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'SA_MAN';

--13. Steven King 사원의 정보를 “Steven King 사원의 급여는 24000달러 입니다” 형식으로 출력하세요.
SELECT '''''' || FIRST_NAME || ' ' || LAST_NAME || ' 사원의 급여는 ' || SALARY || '달러 입니다' || '''''' 출력
