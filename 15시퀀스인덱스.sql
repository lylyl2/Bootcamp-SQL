...
--시퀀스 사용방법 2가지
SELECT DEPTS_SEQ.CURRVAL, FORM DUAL; --NEXTVAL가 한 번 실행이 되고난 이후에 확인이 가능합니다. //X
...
--------------------------------------------------------------------------------
--인덱스(검색을 빠르게 하는 HINT 역할)
--INDEX의 종류로는 고유인덱스, 비고유인덱스가 있습니다.
--고유인덱스는 (PK, UK)를 만들 때 자동으로 생성되는 인덱스입니다.
--비고유인덱스는 일반 컬럼에 지정해서 조회를 빠르게 할 수 있는 인덱스입니다.
--단, INDEX는 조회를 빠르게 하지만, 무작위하게 많이 사용되면 오히려 성능저하를 나타낼 수도 있습니다.
--주로 사용되는 컬럼에서 SELECT절이 속도저하가 있으면, 일단 먼저 고려해볼 사항이 인덱스입니다.
DROP TABLE EMPS;
...
--인덱스 삭제 (삭제하더라도 테이블에 영향을 미치지 않습니다.)
DROP INDEX EMPS_IDX;
...
--고유인덱스 //실제 우리가 만들 일은 없음
CREATE UNIQUE INDEX 인덱스명 ON 테이블명(부착할컬럼); --PK, UK만들 때 자동생성해줍니다(PK, UK 조회할 때 인덱스 효과를 받음)
